...
public class ProjectController extends BaseController {
    ...
    public String saveProject(Project project, String copyCode, HttpSession session) {
        if (StringUtils.isBlank(project.getCode())) {
        ...
        } else if (projectService.checkProjectExist(project.getCode())) {
            //XXX add by kanguangwen
            //检查projectCode是否已经存在
            session.setAttribute("project", project);
            session.setAttribute("message", "项目编码已经存在,添加失败");
            ...
            if (userid == 0) {
                ...
                User user = (User) SessionHolder.getSession().getAttribute("sessionUser");
                long projectId = projectService.saveProject(project, copyCode, user);
                //添加默认的module
                moduleService.save(projectId, "default");
    ...
    public String saveUser(long projectId, long userId, String development, String test, String build,
    ...
    public String updateUser(long projectId, long userId, String development, String test, String build,
                             String production, String admin, HttpSession session) {
        if (StringUtils.isBlank(development) && StringUtils.isBlank(test) && StringUtils.isBlank(build)
            && StringUtils.isBlank(production) && StringUtils.isBlank(admin)) {
            session.setAttribute("message", "请选择用户角色");
