...
public class ProjectService {
    ...
    /**
     * 根据projectId查询项目所有者的userId
     * 
     * @param projectId
     * @return
     */
    public int queryOwnerIdByProjectId(long projectId) {
        String sql = "select OWNER_ID from CONF_PROJECT where ID = ?";
        List<Integer> ownerIds = null;
        try {
            ownerIds = jdbcTemplate.queryForList(sql, Integer.class, projectId);
        } catch (DataAccessException e) {
            LOGGER.error("根据projectId查询项目所有者的userId失败", e);
        }
    ...
    /**
     * 根据projectCode查询项目所有者的userId
     * 
     * @param projectId
     * @return
     */
    public int queryOwnerIdByProjectCode(String projectCode) {
        String sql = "select OWNER_ID from CONF_PROJECT where PROJ_CODE = ?";
        List<Integer> ownerIds = null;
        try {
            ownerIds = jdbcTemplate.queryForList(sql, Integer.class, projectCode);
        } catch (DataAccessException e) {
            LOGGER.error("根据projectCode查询项目所有者的userId失败", e);
        }
        if (!CollectionUtils.isEmpty(ownerIds)) {
            return ownerIds.get(0);
        }
    ...
    public String queryProjectCodeById(Long projectId) {
        String sql = "select PROJ_CODE from CONF_PROJECT where ID = ?";
        List<String> projectCodes = null;
        try {
            projectCodes = jdbcTemplate.queryForList(sql, String.class, projectId);
        } catch (DataAccessException e) {
            LOGGER.error("根据ID查询项目编码失败", e);
        }
    ...
    public Long queryProjectIdByCode(String projectCode) {
        String sql = "select ID from CONF_PROJECT where PROJ_CODE=?";
        List<Long> ids = null;
        try {
            ids = jdbcTemplate.queryForList(sql, Long.class, projectCode);
        } catch (DataAccessException e) {
            LOGGER.error("根据项目编码查询项目ID失败", e);
        }
    ...
    public String queryProjectNameByCode(String projectCode) {
        String sql = "select PROJ_NAME from CONF_PROJECT where PROJ_CODE=?";
        List<String> projectNames = null;
        try {
            projectNames = jdbcTemplate.queryForList(sql, String.class, projectCode);
        } catch (DataAccessException e) {
            LOGGER.error("根据项目编号查询项目名称失败", e);
        }
    ...
    @Transactional
    public long saveProject(Project project, String copyCode, User user) {
    ...
    public List<User> queryUsers(long projectId, int offset, int limit) {
        ...
        //XXX edit by spccold  admin不应该出现在项目管理的添加项目成员中
        String sql = "SELECT a.ID, a.USER_CODE, a.USER_NAME FROM CONF_USER a WHERE a.ID NOT IN "
                     + "(SELECT b.USER_ID FROM CONF_PROJECT_USER b WHERE b.PROJ_ID=?) AND a.DELETE_FLAG=0 AND a.USER_CODE != 'admin' order by a.ID limit ?,?";
    ...
    @Transactional
    public void saveUser(long projectId, long userId, String development, String test, String build, String production,
                         String admin) {
        ...
        if (StringUtils.isNotBlank(admin)) {
            //XXX edit by spccold CONF_PROJECT_USER_ROLE中没必要记录role_code为admin的数据
     ...
     * @param userId
     */
    public List<Project> queryProjectForUser(int offset, int limit) {
    ...
    */
     ...
     * @param userId
     */
    public List<Project> queryProjectForUserNoPager(User user) {
        if ("admin".equals(user.getUserCode())) {
            String sql = "SELECT distinct b.ID, b.PROJ_CODE, b.PROJ_NAME FROM CONF_PROJECT_USER a, CONF_PROJECT b "
                         + "WHERE a.PROJ_ID = b.ID AND b.DELETE_FLAG = 0 order by b.ID desc";
            List<Project> projects = jdbcTemplate.query(sql, new RowMapper<Project>() {
            ...
            String sql = "SELECT distinct b.ID, b.PROJ_CODE, b.PROJ_NAME FROM CONF_PROJECT_USER a, CONF_PROJECT b "
                         + "WHERE a.PROJ_ID = b.ID and a.USER_ID=? AND b.DELETE_FLAG = 0 order by b.ID desc";
            List<Project> projects = jdbcTemplate.query(sql, new RowMapper<Project>() {
     ...
     * @param userId
     */
    public long queryProjectCountForUser() {
    ...
    */
    ...
    public void updateVersion(Long projectId) {
        String sql = "update CONF_PROJECT set DEVELOPMENT_VERSION=DEVELOPMENT_VERSION+1,PRODUCTION_VERSION=PRODUCTION_VERSION+1,"
                     + "TEST_VERSION=TEST_VERSION+1,BUILD_VERSION=BUILD_VERSION+1 where ID=?";
    ...
    private void copyProjConfig(long projId, String projCode, String userCode) {
        ...
        for (Map<String, Object> module : modules) {
            ...
            sql = "SELECT b.CONFIG_KEY, b.CONFIG_VALUE, b.CONFIG_DESC, b.CONFIG_TYPE,b.VISABLE_TYPE FROM CONF_PROJECT a, CONF_PROJECT_CONFIG b "
            ...
            for (Map<String, Object> conf : configs) {
                configService.insertConfig((String) conf.get("CONFIG_KEY"), (String) conf.get("CONFIG_VALUE"),
                    (String) conf.get("CONFIG_DESC"), (String) conf.get("CONFIG_TYPE"),
                    (String) conf.get("VISABLE_TYPE"), projId, moduleId, userCode);
            }
        }
    }
