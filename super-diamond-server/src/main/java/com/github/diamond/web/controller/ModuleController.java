...
public class ModuleController extends BaseController {
	...
	public String save(String type, Long projectId, String name,HttpSession session) {
		long moduleId = moduleService.save(projectId, name);
		if(moduleId == -1){
		    session.setAttribute("message", "相同的模块名已经存在，不能添加！");
		}else if(moduleId == -2){
		    session.setAttribute("message", "ALL为系统保留字段，不能添加！");
		}
